#!/usr/bin/env python3
"""Po-Lisp — Poetic / Poetry / Poem / Poiesis Lisp.

詩のための Lisp。ひとつの機能に、ひと文字。
...
@ = < > + は5つの基本関数、? と ' は2つの特殊形式。あわせて7つの公理。
\\ と * もまた特殊形式で、* は再帰のための自己命名。
! と $ は公理ではなく、橋となる関数。
これらと ( ) は、それぞれ1文字でひとつのトークンなので、空白は要らない。

    (<'(雪 が 降る))              -> 雪
    ((\\(x)(+x(+x())))'木)         -> (木 木)
    (*f(\\(x y)(?((@x)y)(t(+(<x)(f(>x)y))))))   ; 再帰する関数
    (!'(<'(雪 が 降る)))          -> 雪

英数字やかなは、データのためだけに空いている。構文はすべて記号が引き受ける。

7文字だけの核は polisp_min.py にある。
同じ言語を語で書いたもの（語の版の段階的な実装）は polisp_word_eval.py。

使い方:
    python3 polisp.py                           対話モード
...
LAMBDA, LABEL, EVAL, APPLY = "\\", "*", "!", "$"
OPS = ATOM + EQ + CAR + CDR + CONS + COND + QUOTE + LAMBDA + LABEL + EVAL + APPLY
...
def tokenize(src):
    ...
    while i < n:
        ...
        if c == ";":
            ...
            i += 1
        elif c in "()" or c in OPS:   # 記号と括弧は1文字で1トークン
            tokens.append(c)
            i += 1
        else:
            j = i
...
def parse(tokens, pos=0):
    ...
    ( の直後の ' も略記なので、('t 'a) は ((' t) (' a)) というデータになる。
    """
    if pos >= len(tokens):
...
def parse_top(tokens, pos=0):
    ...
    if pos < len(tokens) and tokens[pos] == ":def":
        ...
        name = tokens[pos + 1]
        if name in ("(", ")") or name in (QUOTE, COND, LAMBDA, LABEL):
            raise LispError(":def のあとには名前を書きます（特殊形式は覆せません）")
...
def write(x):
    ...
    if isinstance(x, Closure):
        ...
        if x.name:
            return "#<%s %s (%s)>" % (LAMBDA, x.name, params)
        return "#<%s (%s)>" % (LAMBDA, params)
        ...
        if is_symbol(op) and op in SPECIAL:
            ...
            if op == LABEL:
                ...
                if not is_symbol(args[0]):
                    raise LispError("* の第1引数は名前です: %s" % write(args[0]))
...
def check_params(params):
    if not is_list(params):
        raise LispError("仮引数はリストで書きます: %s" % write(params))
    for p in params:
        if not is_symbol(p):
...
def prim_car(xs):
    if not is_list(xs) or xs == NIL:
        raise LispError("<: 空でないリストが必要です: %s" % write(xs))
    return xs[0]
...
def prim_cons(x, xs):
    if not is_list(xs):
        raise LispError("+ の第2引数はリストでなければなりません: %s" % write(xs))
    return (x,) + xs
...
def global_env():
    ...
    def prim_apply(fn, args):
        if not is_list(args):
            raise LispError("$ の第2引数はリストです: %s" % write(args))
    ...
    env.define(T, T)                       # 空リストは () と書く（nil はない）
    return env


BUILTIN_NAMES = frozenset((ATOM, EQ, CAR, CDR, CONS, EVAL, APPLY, T))
...
さらにふたつの特殊形式:
  ...
  (* 名 式)          式の中から自分自身を名前で呼べる ── 再帰はここから

ふたつの橋（公理ではない関数）:
  (! e a)           データ e を式として評価する。a は ((名前 値) ...) の連想リスト
  ($ f xs)          関数 f を、評価済みの引数のリスト xs に適用する

11個の記号と ( ) はそれぞれ1文字で1トークン。空白はあってもなくてもいい。
英数字やかなは、データのためだけに空いている。
...
def repl():
    ...
    print("Po-Lisp — @ = < > + ? ' \\ * ! $ の11文字。:help で説明、:quit で終了。")
    buf = ""
    while True:
        try:

...
- **ふたつの橋** `!` `$` ── 公理ではない関数。McCarthy が普遍関数（universal function）と呼んだもの。
  データを式として評価する／関数を適用する
...
凡例: `x` `y` は値（アトムでもリストでもよい）、`xs` はリスト（`x` の複数形の s）、
`e` は式、`p` は条件、`f` は関数、`a` は連想リスト。どれも説明のための仮の名前で、言語の一部ではない。
